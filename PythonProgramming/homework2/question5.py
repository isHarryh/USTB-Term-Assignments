...
def get_map(key):
    ...
    remaining = [c for c in reversed(alphabet) if c not in key]
    full_key = key + remaining
    return {alphabet[i]: full_key[i] for i in range(len(alphabet))}

with open('encrypt.txt', 'r', encoding='utf-8') as f_in:
    text = f_in.read()

char_map = get_map(key)

with open('output.txt', 'w', encoding='utf-8') as f_out:
    for c in text:
        if c in char_map:
            f_out.write(char_map[c])
        else:
            f_out.write(c)
