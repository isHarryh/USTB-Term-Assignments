...
class_conditional_probs = {'omega_1': norm(loc=-2, scale=0.5), 'omega_2': norm(loc=2, scale=2)}
risk_loss = {'alpha_1': {'omega_1': 0, 'omega_2': 6}, 'alpha_2': {'omega_1': 1, 'omega_2': 0}}
...
conditional_risks_alpha_1 = [calculate_conditional_risk(x)[0]['alpha_1'] for x in x_values]
conditional_risks_alpha_2 = [calculate_conditional_risk(x)[0]['alpha_2'] for x in x_values]

plt.plot(x_values, conditional_risks_alpha_1, label='Conditional Risk for alpha_1', linestyle='-', color='blue')
plt.plot(x_values, conditional_risks_alpha_2, label='Conditional Risk for alpha_2', linestyle='-', color='red')

for i in range(len(observations)):
    if classification_result[i] == 'alpha_1':
        plt.plot(observations[i], conditional_risks_list[i]['alpha_1'], marker='o', color='blue')
    else:
        plt.plot(observations[i], conditional_risks_list[i]['alpha_2'], marker='s', color='red')
